...
class GenerateAmplicons:
    ...
    def fasta_reference_pathway_list(self) -> list:
        ...
        return [(file, os.path.basename(file)) for file in file_pathway_list if os.path.splitext(file)[1].lower() in valid_extensions]

    def fasta_reference_file_to_list(self) -> list:
        '''
        return tuple as [(fasta pathway)]
        '''
        return list((self.fasta_pathway, os.path.basename(self.fasta_pathway)))
    
    def read_referecne_fasta_list(self, fasta_pathway_list: list) -> dict:
        '''
        ...
        for reference_sequence_defline, reference_sequence_values in ref_seq_dict.items():
            ...
            for primer_name, primer_seqs in primer_scheme_dict.items():
                ...
                index_amplicon_list = primer_index_for_amplicon_seq(
                    ref_seq=reference_sequence_seq,
                ...
                if not index_amplicon_list:
                    ...
                    amplicon_dict[reference_fasta_basename] = {defline: amplicion_seq}
    ...
    def write_amplicion_fasta_files(self, amplicon_storage_dir_pathway: str, amplicon_dict: dict) -> None:
        ...
        for ref_name, seq_dict in amplicon_dict.items():
            output_fasta = os.path.join(amplicon_storage_dir_pathway, ref_name + ".fasta")
            with open(output_fasta, 'w') as wf:
                for defline, amplicon_sequence in seq_dict.items():
                        wf.write(f">{defline}\n")
...
def opts():
    ...
    amplicon_parser = argparse.ArgumentParser(prog="SEWAGE amplicon",
                                     ...
                                     add_help=True,
                                     epilog="Generate amplicon sequence files")
...
def GenerateAmplicon_MainWorkflow():
    
    sewage_dir = "/scicomp/home-pure/ofx5/GitHub/SEWAGE"
    args = opts()

    sewage_amplicon = GenerateAmplicons(args.fasta, 
                                        args.scheme, 
                                        sewage_dir, 
                                        args.output, 
                                        args.pathway)
    ...
    if sewage_amplicon.Check_fasta_reference_input() == True:
        fasta_pathway_list = sewage_amplicon.fasta_reference_pathway_list()
    elif sewage_amplicon.Check_fasta_reference_input() == False:
        fasta_pathway_list = sewage_amplicon.fasta_reference_file_to_list()
    elif sewage_amplicon.Check_fasta_reference_input() == None:
        raise TypeError("Something went wrong")
    
    # Create storage dir and return pathway
    amplicon_storage_dir_pathway = sewage_amplicon.make_amplicon_storage_dir()
    
    reference_sequence_dict = sewage_amplicon.read_referecne_fasta_list(fasta_pathway_list)
    ...
    print(f"Fin with amplicons...", file=sys.stderr)

if __name__ == "__main__":
    GenerateAmplicon_MainWorkflow()